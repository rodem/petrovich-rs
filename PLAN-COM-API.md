...
- Реализация: head-word + `lastname(gender авто, head, case)` ядра;
  винительный — эвристика одушевлённости (см. `adapter.rs`);
  пол head — `detect_gender`, fallback мужской. Расхождения фиксирует
  дифф-харнес на датасете заказчика.
