...
### Этап C4. Верификация ✅ 2026-09-19 + расширение 2026-09-19 (дока v4.1)
Источник истины: `github.com/rodem/padeg-docs`. По нему: ProgID
`PadegUCA.Declension` (первичный), `GetSex → -1`, коды `-1/-2` в `scode`,
правило IF «последнее слово — фамилия», C-экспорты с буферным протоколом,
COM-методы `GetFIOPadeg`/`GetIFPadeg*`/`SeparateFIO`/словарь (DISPID 7–13),
числительные — следующий этап. Ручного прогона из 1С по-прежнему нет.
